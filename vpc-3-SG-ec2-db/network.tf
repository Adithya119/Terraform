#This containes vpc, subnets, SG's, igw, route_table, route_table_association
...
resource "aws_subnet" "subnets" {
  count = length(var.subnet_cidrs)      # this is for loop => for count = 4
  vpc_id     = aws_vpc.primary-vpc.id   # id is an attribute of vpc
...
resource "aws_security_group" "Web-SG" {   
  vpc_id      = aws_vpc.primary-vpc.id    # see that while manually creating SG, it asks for vpc
  name        = "Web-SG"
...
resource "aws_route_table" "public_route_table" {       
  ...
  route {                                         # route block
  cidr_block = local.anywhere                        # will allow all connections from igw-1 to this table
  gateway_id = aws_internet_gateway.igw-1.id         # attaching this route table to igw-1
  }
...
# associating subnets to appropriate route_tables
